...
export const userRegistration = async (
  ...
  try {
    ...
    if (existingUser) {
      return next(
        new ValidationError(
          "Email already exists. You can't register with same email. Please use different email.",
        ),
      );
    }
...
export const verifyUser = async (
  ...
  try {
    ...
    if (!name || !email || !password || !otp) {
      return next(
        new ValidationError("Missing required fields for verification"),
      );
    }
    const existingUser = await (prisma as any).user.findUnique({
      where: { email },
...
export const refreshToken = async (
  ...
  try {
    ...
    // REUSE DETECTION: Token in cookie doesn't match the one in our DB, which means the token has been used before and a new token has been issued. This is a strong indication of token theft, so we immediately invalidate all sessions for this user.
    ...
    const newAccessToken = jwt.sign(
      { userId: session.user.id, role: "user" },
