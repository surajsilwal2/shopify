...
export const validateRegister = async (data: any, userType: 'user' | 'seller') => {
...
export const verifyOtp = async (email: string, otp: string) => {
  ...
  if (attempts >= 3) {
    throw new ValidationError(
      "Too many incorrect codes. Try again in 30 minutes.",
    );
  }
...
export const handleForgetPassword = async (req: Request, res: Response, next:NextFunction, userType: 'user' | 'seller') => { 
 try {
   ...
   const user = userType === 'user' && await (prisma as any).user.findUnique({ where: { email } });
   if (!user) {
...
export const handleVerifyForgetPasswordOtp = async(req: Request, res: Response, next: NextFunction) => {
  try {
